...
(7월 30일과 8월 6일 데이터는 24시간 연속적인 데이터가 확보되지 않아 그래프 시각화에서는 제외하였지만, 학습 데이터로는 부분적으로 활용하여 최종적으로 8일치의 패턴을 모델 학습에 사용하였다.) 위 그래프는 날짜/시간별 트래픽의 변화 정도를 나타낸 그래프이다. 각 날짜의 데이터마다 http, ssl, conn 등의 다양한 로그 파일이 생성되었으며, 로그파일들에서 통계적 특징을 추출하기 위한 자동스크립트를 작성하여 이용하였다. (로그 파일은 크기 제한 때문에 업로드하지 않았다.)
